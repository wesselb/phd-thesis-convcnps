...
% Translation equivariance is an important inductive bias for many learning problems. including time series, spatial data, and images.
ConvNPs rely on convolutional neural networks rather than multi-layer perceptrons. % which makes convolutional neural processes more data efficient than their non-convolutional predecessors.
...
% Directly parametrising these dependencies poses simple and computationally cheap alternative approach.

Third, we propose \emph{autoregressive conditional neural processes} (AR CNPs).
AR CNPs train a neural process without any modifications to the model or training procedure and, at test time, roll out the model in an autoregressive fashion. % to produce improved predictions.
