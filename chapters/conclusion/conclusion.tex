...
% Convolutional Conditional Neural Process (ConvCNP; \cref{\xrprefix{mod:convcnp}}), the Convolutional Gaussian Neural Process (ConvGNP; \cref{\xrprefix{mod:convgnp}}),
% and the Fully Convolutional Gaussian Neural Processes (FullConvGNP; \cref{\xrprefix{mod:fullconvgnp}}).
...
% The more powerful idea and the transferable tool is that convolutional deep sets
% can be used to generally approach spatial, temporal or spatio--temporal data problems in a simple and effective way.
...
%To effectively deploy convolutional deep sets, the data must have one, two, or---at a stretch---three-dimesional inputs, and one must carefully tune the discretisation to the problem at hand.
%
...
%By retaining a tractable objective, GNPs can be simpler and computationally cheaper than LNPs.
In problems where the data is roughly Gaussian, GNPs are a promising new model class to explore.
%For example, in the Gaussian synthetic experiments (\cref{\xrprefix{sec:experiments:synthetic}}), predator--prey experiments (\cref{\xrprefix{sec:experiments:predprey}}) and electroencephalography experiments (\cref{\xrprefix{sec:experiments:eeg}}), a GNP topped the leaderboard.
%However, when the data exhibits non-Gaussian aspects, such as in the non-Gaussian synthetic experiments (\cref{\xrprefix{sec:experiments:synthetic}}), LNPs offer clear advantages.
...
%$A worthwhile mention are GNPs with the so-called \texttt{kvv}-covariance , which were introduced by the colleagues of the author and the author \parencite{Markou:2022:Practical_Conditional_Neural_Processes_for_Tractable}, but not discussed in this thesis.
...
% The simplicity and flexibility of AR CNPs comes at the cost of increased computational expense at test time and the lack of consistency.
% Future research will reveal whether or where this trade-off is appropriate.
...
%to arrive at a solution suited for a particular application
...
%All approaches can deal with arbitrary missing data.
%However, when the outputs are multidimensional and arbitrary dimensions can be missing, convolutional deep sets offer the best scalability.
