...
\paragraph{Outline}
%In \cref{sec:experiments:introduction}, we give an overview of all models that will be compared.
...
%We first analyse the results for the Gaussian experiments.
...
% When the ACNP and CNP are evaluated outside of the training range, their performance deteriorates.
The ConvCNP, on the other hand, performs equally well in the interpolation and OOID interpolation tasks.
% has equally good interpolation performance beyond the training range as inside the training range.
...
% says that the hare--lynx data set is scanned from a graph originally published by \textcite{MacLulich:1937:Fluctuations_in_the_Numbers_of_the_Varying_Hare}.
...
\looseness=-1
% between our stochastic version of the Lotka--Volterra equations and the real hare--lynx data.
...
% Similarly, the ConvNP trained and evaluated with the ELBO objective produces the smallest uncertainty for the simulated data. 
% Even though its performance on the simulated data is reasonable,
% the ConvNP's predictions on the real data completely miss the held-out data.
...
%In environmental modelling, a large effort is spent on simulating the atmospheric equations of motion to make predictions for the future.
...
%For the VALUE task, we follow the setup of \citeauthor{Vaughan:2022:Convolutional_Conditional_Neural_Processes_for}'s baseline experiment.
%and compare against existing statistical downscaling approaches in a standardised experimental setup.
%The standardised setup is experiment 1a from the VALUE framework \parencite{Maraun:2015:VALUE_A_Framework_to_Validate}.
%Following experiment 1a from the VALUE framework, % \citeauthor{Vaughan:2022:Convolutional_Conditional_Neural_Processes_for}'s baseline experiment,
...
%\parencite{Danielson:2011:Global_Multi-Resolution_Terrain_Elevation_Data}  high-resolution for MLP models
...
%\url{https://doi.org/10.5066/F7J38R2N} high-resolution for MLP models
...
\begin{table}[t]
    ...
    %         & $\mathbf{1.00} \, {\scriptstyle \pm 0.04}$
    %         & $\shortminus 1.62 \, {\scriptstyle \pm 0.01}$
    %         & $\mathbf{0.96} \, {\scriptstyle \pm 0.02}$ \\
    %     ConvGNP (MLP)
    %         & $\mathbf{\shortminus 1.63} \, {\scriptstyle \pm 0.00}$
    %         & $\mathbf{0.97} \, {\scriptstyle \pm 0.03}$
    %         & $\mathbf{\shortminus 1.45} \, {\scriptstyle \pm 0.00}$
    %         & $1.07 \, {\scriptstyle \pm 0.02}$ \\
    %     Best VALUE PP model
    %         &
    %         & $1.30 \, {\scriptstyle \pm 0.08}$ \\
    %     Best VALUE model
    %         &
    %         & $\mathbf{1.08} \, {\scriptstyle \pm 0.09}$ \\
    %     \bottomrule
    % \end{tabular}
...
%The best performing approach in this comparison is shown in \cref{tab:climate_downscaling_results}.
%Many of these approaches, however, like \note{explain quantile mapping (MOS; REF)}, are not at all comparable to the neural process approach.
...
\begin{figure}[t]
    ...
    \caption[
        ...
        %The high-resolution context data $D\ss{hr}$ also consists of the station observations and the \SI{1}{km}--resolution elevation data.
...
% In the autoregressive sampling procedure (\cref{\xrprefix{proc:ar}}),
% the model is evaluated at one target point at a time.
% Consider such a target point.
% Let us zoom in on how the prediction for that target point depends on all input to the model: earlier AR samples from the model, the coarse-grained ERA-Interim reanalysis data, and the \SI{1}{km}--resolution elevation data.
% The earlier AR samples from the model represent fine-grained weather predictions.
% Since the local weather changes on a fairly short spatial scale, it seems reasonable that the prediction for the target point more strongly depends on earlier samples nearby, and less so on earlier samples farther away.
% The coarse-grained ERA-Interim reanalysis data, on the other hand, includes features which vary on a long spatial scale.
% For these features, it seems reasonable to conclude a dependence across all of Germany.
% Lastly, for the \SI{1}{km}--resolution elevation data,
% the prediction for the target point likely depends on detailed orographic information nearby,
% but more on general orographic features farther away.
% The general trend is that the prediction likely depends on short-length-scale details nearby the target point, and that this dependence weakens as we move farther away, starting to depend more on broad-stroke long-length-scale details.
% It is this trend that the architecture attempts to exploit.
...
%This, in turn, allows the CNN to have a far-ranging discretisation, capturing long-scale dependencies.
...
%and captures medium-scale dependencies.
...
% Similarly, the medium-resolution discretisation dovers a medium-sized square covering $5^\circ$  more than the most extremal target points.
% The low-resolution grid, however, covers all of Germany.
...
%The central assumption of the architecture is that predictions for target points increasingly depend on broad-stroke features of context data farther away rather than high-resolution details.
%more on context data farther away depends more on broad-stroke features rather than high-resolution details.
...
%To this end, improving calibrating by appropriately inflating the uncertainty post-hoc could be further investigated.
